...
```
질문 검증 → 스탯/문맥 추출(LLM) → 문맥 병합(멀티턴) → 역할·주제 영웅 확인 또는 고정 답변 응답(주제 무관 질문)
→ 검색 → 추천 영웅 판단(조건부, LLM) → 답변 생성(일반 텍스트/상성 카드/추천 카드 분기, "간단히"/"자세히" 반영)
→ 추천 질문 생성(조건부) → 응답 포맷팅으로 이어지는 15개 노드 상태 그래프(StateGraph) 구현
```
